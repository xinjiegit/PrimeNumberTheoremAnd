...
theorem Dusart_thm :
  ∀ x : ℝ,
    x ≥ 10726905041 →
      ∃ p : ℕ, Nat.Prime p ∧ x - x / 2831400 < (p : ℝ) ∧ (p : ℝ) ≤ x := by
  sorry
...
abbrev X₁ := 10726905041

lemma hn_ge_X₀_sq {n : ℕ} (hn : n ≥ X₁ ^ 2) : n ≥ X₀ ^ 2 := by
  have h : (X₀ ^ 2 : ℕ) ≤ X₁ ^ 2 := by decide
  exact le_trans h hn
...
theorem exists_p_primes {n : ℕ} (hn : n ≥ X₁ ^ 2) :
  ...
  have hx1_ge : x * (1 + ε) ≥ X₀ := (hsqrt_ge hn0).trans (le_mul_of_one_le_right hx_pos.le
    (by grind))
  have hx2_ge : x * (1 + ε) ^ 2 ≥ X₀ := (hsqrt_ge hn0).trans (le_mul_of_one_le_right hx_pos.le
    (by nlinarith [sq_nonneg ε]))
  obtain ⟨p₀, hp₀_prime, hp₀_lb, hp₀_ub⟩ := Dusart_interval (x := x) (hsqrt_ge hn0)
...
theorem exists_q_primes {n : ℕ} (hn : n ≥ X₁ ^ 2) :
  ...
  have hy₀_ge : n / (1 + ε) ^ 3 ≥ X₀ := by
    calc n / (1 + ε) ^ 3 = x ^ 2 / (1 + ε) ^ 3 := by rw [hn_eq_x2]
      ...
      _ ≥ X₀ ^ 2 / 2 := by
        apply div_le_div_of_nonneg_right (sq_le_sq' (by linarith) (hsqrt_ge hn0))
        norm_num
      _ ≥ X₀ := by norm_num
...
noncomputable def Criterion.mk' {n : ℕ} (hn : n ≥ X₁ ^ 2) : Criterion where
  ...
  hn := by
    have hn0 : n ≥ X₀ ^ 2 := hn_ge_X₀_sq hn
    exact le_trans (by decide : 1 ≤ 89693 ^ 2) hn0
  ...
  h_crit := by
    ...
    grw [prod_q_ge hn, prod_p_ge hn, h₁]
...
theorem L_not_HA_of_ge (n : ℕ) (hn : n ≥ 10726905041 ^ 2) : ¬HighlyAbundant (L n) :=
...
/-
BUILD: SUCCESS
Build command: Lcm_T1_A1.lean
Build attempts: 5
Start time: Wed Jan 28 18:26:14 PST 2026
End time: Wed Jan 28 18:49:48 PST 2026
Elapsed time: 0:23:34
Final bound C: 10726905041 ^ 2
Confirmations:

sorry occurs exactly once (in Dusart_thm)
no axiom or admit tokens in Lcm_T1_A1.lean
Dusart_thm, HighlyAbundant, L, and theorem name L_not_HA_of_ge unchanged except for bound C
-/
